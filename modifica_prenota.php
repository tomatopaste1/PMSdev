...
echo "</td></tr></table><br>
<button class=\"gobk\" type=\"submit\" name=\"torna\" value=\"".mex("NO",$pag)."\"><div>".mex("NO",$pag)."</div></button>
</div></form>";
} # fine if ($cancella_c != "SI")
else {

if ($priv_canc_permanente == "n" or ($priv_canc_permanente == "c" and !$cancella_permanentemente)) $cancperm = 0;
else $cancperm = 1;
...
for ($num_idpr = 0 ; $num_idpr < $num_id_prenota ; $num_idpr++) {
$id_prenota = $id_prenota_idpr[$num_idpr];
$testo = cancella_prenotazione($id_prenota,$cancella_pagato,$metodo_pagamento,$priv_mod_pagato,1,$anno,$PHPR_TAB_PRE,$id_utente,$stato_canc,"",1,$modo_canc);
echo $testo;
} # fine for $num_idpr
...
if (!$n_valuta_caparra and $tipo_val_caparra and $tipo_val_caparra != "perc_tar") {
...
} # fine if (!$n_valuta_caparra and !strcmp($n_caparra,"") and $tipo_val_caparra and $tipo_val_caparra != "perc_tar")
...
<input type=\"hidden\" name=\"tipo_val_sconto\" value=\"".htmlspecialchars($tipo_val_sconto)."\">
...
$dati_form_modifica .=  "<input type=\"hidden\" name=\"n_confermato\" value=\"".htmlspecialchars($n_confermato)."\">
<input type=\"hidden\" name=\"modo_aggiorna_pagato\" value=\"".htmlspecialchars($modo_aggiorna_pagato)."\">
<input type=\"hidden\" name=\"valuta_aggiorna_pagato\" value=\"".htmlspecialchars($valuta_aggiorna_pagato)."\">
<input type=\"hidden\" name=\"n_pagato\" value=\"".htmlspecialchars($n_pagato)."\">";
...
$tipo_val_sconto_orig = $tipo_val_sconto;
...
} # fine else if ($cambia_cat_pers_ca[$numca])
aggiorna_letti_agg_in_periodi($dati_cap,$numca,$num_letti_agg,$inizioperiodo,$fineperiodo,$n_settimane_costo[$numca],$n_moltiplica_costo[$numca],"","",$dati_cat_pers_corr,$catpers_ca_corr);
} # fine if ($$numcostoaggiuntivo == "SI")
} # fine if ($$numcostoaggiuntivo == "SI")
} # fine for $numca
...
if ($dati_cap[$numca]['modo_cp_molt']) {
...
if ($dati_cap[$numca]['cp_molt'][$cp]['esist'] and $cat_persone_corr[$cat_persone_corr['ord'][$cp]]['molt']) {
...
} # fine if ($dati_cap[$numca]['cp_molt'][$cp]['esist'] and $cat_persone_corr[$cat_persone_corr['ord'][$cp]]['molt'])
} # fine for $num1
} # fine if ($dati_cap[$numca]['modo_cp_molt'])
...
for ($num1 = 0 ; $num1 < $dati_cap[$numca]['cp_molt']['num'] ; $num1++) {
$cp = $dati_cap[$numca]['cp_molt']['ord'][$num1];
if ($dati_cap[$numca]['cp_molt'][$cp]['esist'] and $num_letti_agg['catp_tot_max'][$cp]) {
...
} # fine if ($dati_cap[$numca]['cp_molt'][$cp]['esist'] and $num_letti_agg['catp_tot_max'][$cp])
} # fine for $num1
...
$n_sconto_caparra_idpr[$id_prenota] = $n_sconto."#>&".$n_caparra."#>&".$n_commissioni."#>&".$n_valuta_tariffa;
...
if ($mostra_met_paga_caparra == "SI" and strcmp($d_valu_caparra,"")) {
$caparra_valuta = "";
...
} # fine if ($mostra_met_paga_caparra == "SI" and $d_valuta_caparra and $d_valuta_caparra != "~~")
...
if ($n_valuta_caparra == ">".$altre_valute[$num1]['nome']) $sel = " selected";
else $sel = "";
...
echo "<option value=\"caparra\"$sel>".mex("caparra ",$pag)."</option>
 ...
 <input type=\"text\" name=\"n_pagato\" value=\"$n_pagato\" size=\"8\"> ";
if (!$altre_valute['num']) echo $Euro;
else {
if (!strcmp($valuta_aggiorna_pagato,"")) $sel = " selected";
else $sel = "";
echo "<select name=\"valuta_aggiorna_pagato\">
<option value=\"\"$sel>$Euro</option>";
for ($num1 = 0 ; $num1 < $altre_valute['num'] ; $num1++) {
if ($valuta_aggiorna_pagato == $altre_valute[$num1]['nome']) $sel = " selected";
else $sel = "";
echo "<option value=\"".htmlspecialchars($altre_valute[$num1]['nome'])."\"$sel>".$altre_valute[$num1]['nome']."</option>";
} # fine for $num1
echo "</select>";
...
<input type=\"hidden\" name=\"email2_1\" value=\"$demail\">
...
<input type=\"hidden\" name=\"tasso_cambio_paga$num1"."_1\" value=\"".$tasso_cambio_paga[$num1]."\">
