...
var cas_ini = 'rd_'+c_ini+' ';
if (caselle[n2].className == 'rd_'+c_ini || caselle[n2].className.substr(0,cas_ini.length) == cas_ini) {
...
if (tabelle[n0].className == 'm1' || tabelle[n0].className.substr(0,3) == 'm1 ') {
...
xmlhttp.open('GET','tabella.php?id_sessione='+id_sessione+'&dati_prn='+id_prn,true);
xmlhttp.send();
}
else link_elem.setAttribute('title',ArTiPr[id_prn]);
}
...
for (n2 = 1 ; n2 < (caselle.length - 1) ; n2++) {
if (n1 == 0) caselle[n2].className = 'rd_'+(n2 - 1)+' t1top';
else caselle[n2].className = 'rd_'+(n2 - 1);
if (gjs == '16' && direz == 'dx') {
...
function selizona_opt_con_val (id_sel,val) {
var sel_el = document.getElementById(id_sel);
var opts = sel_el.options;
for (var n1 = 0, lopts = opts.length; n1 < lopts; n1++) {
if (opts[n1].value == val) {
sel_el.selectedIndex = n1;
return true;
}
}
return false;
} // fine function selizona_opt_con_val
