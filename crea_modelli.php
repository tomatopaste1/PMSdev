...
if ($eliminacampipers or $aggiungicampipers or $eliminacampicond or $aggiungicampicond or $eliminacodpromo or $aggiungicodpromo or $eliminacampickin or $aggiungicampickin) {
unset($crea_modello);
$$nome_form_modello_passa = "SI";
...
if ($$template_name) {
$mostra_form_creazione = "NO";
if (($template_name_show['tpl_type'] != "interconnection" and $priv_crea_pagineweb == "s") or ($template_name_show['tpl_type'] == "interconnection" and $priv_crea_interconnessioni == "s")) {
...
} # fine if (($template_name_show['tpl_type'] != "interconnection" and $priv_crea_pagineweb == "s") or...
break;
...
if (!strstr($option_utenti,"\"$utente_liste\">") and $num_utenti[$utente_liste]) {
if ($utente_liste == $utente_lis) $sel = " selected";
else $sel = "";
echo "<option value=\"$utente_liste\"$sel>$utente_liste</option>";
} # fine if (!strstr($option_utenti,"\"$utente_liste\">") and $num_utenti[$nome_utente])
...
for ($numca = 0 ; $numca < $dati_ca['num'] ; $numca++) {
if (strtoupper($costi_aggiuntivi_mostra[$dati_ca[$numca]['id']]) == $SI) $checked = " checked";
else $checked = "";
...
if (!$costi_aggiuntivi_mostra["array_esistente"]) $checked = " checked";
...
if (!strcmp($orig_prenota,"")) $orig_prenota = $origine_prenotazione;
...
if (!$chiedi_nazione) $chiedi_nazione = $NO;
...
if (!$chiedi_valutapagamento) $chiedi_valutapagamento = $SI;
...
if (strtoupper($metodi_pagamento_da_chiedere[$metodo]) == $SI) $metodi_pagamenti_chiesti[$metodo] = 1;
...
if ($campi_pers_comm or strcmp($ins_campi_form_personalizzati[$num1],"")) {
...
echo "</select><br>";
} # fine if ($campi_pers_comm or strcmp($ins_campi_form_personalizzati[$num1],""))
...
if (strcmp($nome_contratto_ut[$num_contr],"")) $nomi_contr_salva[$num_contr] .= " (".$nome_contratto_ut[$num_contr].")";
...
if ($chiedi_campi_form_doc_condizioni[$num1] == "op_".$campo_pers_cliente) $campo_pers_trovato = 1;
...
$sel_DIS = "";
$sel_SI = "";
if (!$mostra_quadro_disponibilita or strtoupper($mostra_quadro_disponibilita) == $SI."_".$NO) $sel_DIS = " selected";
else {
...
$sel_REG2 = "";
$sel_PERS = "";
if (!strcmp($raggruppa_quadro_disponibilita_con_regola_2,"") and !strcmp($raggruppa_quadro_disponibilita_con_persone,"")) $sel_REG2 = " selected";
else {
...
} # fine else if (!strcmp($raggruppa_quadro_disponibilita_con_regola_2,"") and !strcmp(,"$raggruppa_quadro_disponibilita_con_persone"))
...
if (!$allinea_disponibilita_con_arrivo or strtoupper($allinea_disponibilita_con_arrivo) == $SI or $allinea_disponibilita_con_arrivo == "SI") { $sel_SI = " selected"; $sel_NO = ""; }
else { $sel_NO = " selected"; $sel_SI = ""; }
...
if ($valori_tema[$num2]['img']) $js_opz_tema .= " ".upload_hd_img_form("valtema$num2",$tablepersonalizza,$num2,1);
$js_opz_tema .= "<br>";
} # fine for $num2
...
if ($cancella_modelli == "SI" and $mostra_form_creazione != "NO") {
...
} # fine if ($cancella_modelli == "SI" and $mostra_form_creazione != "NO")




if ($importa_modelli == "SI" and $mostra_form_creazione != "NO") {
if (@get_magic_quotes_gpc()) {
...
} # fine if ($importa_modelli == "SI" and $mostra_form_creazione != "NO")
