...
if ($opz_cerc_pren != "arr" and $opz_cerc_pren != "part" and $opz_cerc_pren != "canc") $opz_cerc_pren = "";
...
if ($pcanc and !$cerca_prenota) $pcanc = "&amp;pcanc=1";
else $pcanc = "";
...
$caparra_p = punti_in_num($caparra,$stile_soldi);
if ($caparra) {
if (!$valuta_caparra) $caparra_valuta_TOT[$Euro] += $caparra;
else {
$valuta_caparra = explode("<",$valuta_caparra);
$cambio_caparra = $valuta_caparra[1];
$arrotond_caparra = $valuta_caparra[2];
...
$costo_tot_p = punti_in_num($costo_tot,$stile_soldi);
if ($costo_tot) {
if (!$valuta_tariffa) $costo_tot_vauta_TOT[$Euro] += $costo_tot;
else {
$valuta_tariffa = explode("<",$valuta_tariffa);
...
$costo_agg_parziale = (double) calcola_prezzo_totale_costo($dati_cap,$numca,$id_data_inizio,$id_data_fine,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$costo_tariffa,$tariffesettimanali,($costo_tariffa + $costo_agg_tot - $sconto),$caparra,$numpersone,$dati_cap[$numca]['cat_pers'],0,$costo_escludi_perc);
...
if ($col_gr_caparra[$num2] == "SI") { $colonne_costi .= $spazio.str_replace("<br>"," ",$caparra_p); $spazio = $separatore; }
...
if ($mostra_tab_costi != "NO" or $conv_valuta_cassa) {
if ($priv_vedi_tab_costi == "p" or $priv_vedi_tab_costi == "g") {
$condizione_costi_propri = "and ( utente_inserimento = '$id_utente'";
if ($priv_vedi_tab_costi == "g") {
reset($utenti_gruppi);
...
} # fine if ($mostra_tab_costi != "NO" or $conv_valuta_cassa)


if ($conv_valuta_cassa and ($priv_ins_entrate == "s" or $priv_ins_spese == "s")) {
...
} # fine if ($conv_valuta_cassa and ($priv_ins_entrate == "s" or $priv_ins_spese == "s"))
...
if ($valuta_corr != $Euro and !$altre_valute_viste[$valuta_corr]) {
...
} # fine if ($valuta_corr != $Euro and !$altre_valute_viste[$valuta_corr])
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"periodi\">
<input type=\"hidden\" name=\"aggiungi\" value=\"1\">";
if ($mostra_form_agg_per) $form_agg_per .= "<input type=\"hidden\" name=\"continua\" value=\"1\"><span class=\"colinfo\">".mex("Attenzione",$pag)."</span>: ".mex("i periodi aggiunti non potranno più essere eliminati",$pag).".<br><br><br>";
...
} # fine if ($dati_ca[$num1]['cp_molt']['num'])
else $cp_molt = "";
...
if ($idregole and !$torna_alla_tab and ($priv_mod_reg1 != "n" or $priv_mod_reg2 == "s")) {
$aggiorna_ic_tar = 0;
$aggiorna_ic_disp = 0;
$tabelle_lock = array($tableregole);
...
if (strcmp($tariffa_chiusa,"") and ($attiva_tariffe_consentite != "n" and $tariffe_consentite_vett[substr($tariffa_chiusa,7)] != "SI")) $continua = 0;
...
if (strcmp($tariffa_per_app,"") and $attiva_tariffe_consentite != "n" and $tariffe_consentite_vett[substr($tariffa_per_app,7)] != "SI") $continua = 0;
if (strcmp($tariffa_per_persone,"") and $attiva_tariffe_consentite != "n" and $tariffe_consentite_vett[substr($tariffa_per_persone,7)] != "SI") $continua = 0;
...
} # fine if ($idregole and !$torna_alla_tab and ($priv_mod_reg1 != "n" or $priv_mod_reg2 == "s"))

else {


echo "<h3 id=\"h_rul\"><span id=\"hreg1\">".mex("Tabella con le regole di assegnazione 1 del",$pag)." $anno.</span></h3>
<br><div class=\"tab_cont\">
<table class=\"t1 t1color\" style=\"margin-left: auto; margin-right: auto;\" width=3 border=\"$t1border\" cellspacing=\"$t1cellspacing\" cellpadding=\"$t1cellpadding\">
...
$link_cancella = 1;
if (strcmp($tariffa_chiusa,"")) {
if ($attiva_tariffe_consentite != "n" and $tariffe_consentite_vett[substr($tariffa_chiusa,7)] != "SI") $link_cancella = 0;
...
if ($priv_mod_reg2 == "s") {
echo "<td>";
if ($attiva_tariffe_consentite != "n" and $tariffe_consentite_vett[$numtariffa] != "SI") echo "&nbsp;";
...
echo "<br><br><h3 id=\"h_rul\"><span id=\"hreg3\">".mex("Tabella con le regole di assegnazione 3 del",$pag)." $anno.</span></h3>
<br><div class=\"tab_cont\">
<table class=\"t1 t1color\" style=\"margin-left: auto; margin-right: auto;\" width=3 border=\"$t1border\" cellspacing=\"$t1cellspacing\" cellpadding=\"$t1cellpadding\">
<tr><td>".mex("Tariffa",$pag)."</td>
...
if ($priv_mod_reg2 == "s") {
if ($attiva_tariffe_consentite != "n" and $tariffe_consentite_vett[substr($tariffa,7)] != "SI") $html_tab[$lung] .= "<td>&nbsp;</td>";
...
$html_tab[$lung] .= "</tr>";
} # fine if ($priv_vedi_tab_regole != "p" or $attiva_tariffe_consentite == "n" or $tariffe_consentite_vett[substr($tariffa,7)] == "SI")
} # fine for $num1
if ($html_tab) {
ksort($html_tab);
foreach ($html_tab as $val) echo $val;
...
if ($installazione_subordinata != "SI") echo "<td class=\"t1top\">".mex("Cancella",$pag)."</td>";
echo "</tr>";
...
if ($installazione_subordinata != "SI" and ($priv_mod_reg1 != "n" or $priv_mod_reg2 == "s")) {
echo "<br><div style=\"text-align: center;\">
...
echo "<td class=\"t1top\">".mex("Commento",$pag)."</td>";
if ($id_utente == 1 and $installazione_subordinata != "SI") echo "<td class=\"t1top\">".mex("Modifica",$pag)."</td>";
} # fine if (!$form_tabella)
echo "</tr>";
...
echo "<td style=\"font-size: x-small;\">$commento</td>";
if ($id_utente == 1 and $installazione_subordinata != "SI") echo "<td><a href=\"modifica_app.php?tipo_tabella=$tipo_tabella&amp;anno=$anno&amp;id_sessione=$id_sessione&amp;idappartamenti=".urlencode($idappartamenti)."\">".mex("modifica",$pag)."</a></td>";
echo "</tr>";
} # fine if (!$form_tabella)
else {
echo "<tr>";
...
<td class=\"t1top\">".mex("Descrizione",$pag)."</td>";
if (!$modifica_tab_beni_inv) echo "<td class=\"t1top\">".mex("Mancanti",$pag)."</td>";
if ($id_utente == 1 and $num_tutti_utenti > 1) echo "<td class=\"t1top\">".mex("Utente",$pag)."</td>";
if (!$modifica_tab_beni_inv) echo "<td class=\"t1top\">".mex("Cancella",$pag)."</td>";
echo "</tr>";
...
<td class=\"t1top\">".ucfirst(mex("prenotazioni",$pag))."</td>
...
if (($num_pren_esist2[0] + 1) == $anno and $num_pren_esist2[2]) $num_pren_esist[$num1] = $num_pren_esist2[2];
else {
