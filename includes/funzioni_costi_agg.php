...
function dati_costi_agg_ntariffe ($tablenometariffe,$num_tariffe,$solo_visibili="NO",$ordine_imposto="",$tableappartamenti="",$dati_cat_pers=array()) {
global $LIKE;
...
if ($dati_ca[$num1]['mostra'] == "p") {
$dati_ca[$num1]['mostra'] = "s";
$dati_ca[$num1]['mostra_numpers'] = "s";
} # fine ($dati_ca[$num1]['mostra'] == "p")
else $dati_ca[$num1]['mostra_numpers'] = "n";
$dati_ca[$num1]['moltiplica'] = risul_query($costi,$num1,'moltiplica_ca');
$dati_ca[$num1]['cp_molt_int'] = strstr($dati_ca[$num1]['moltiplica'],"<");
if ($dati_ca[$num1]['cp_molt_int']) {
$catpers_molt = explode("<",$dati_ca[$num1]['moltiplica']);
$dati_ca[$num1]['moltiplica'] = $catpers_molt[0];
$dati_ca[$num1]['modo_cp_molt'] = $catpers_molt[1];
$dati_ca[$num1]['cp_molt']['num'] = (count($catpers_molt) - 2);
for ($num2 = 0 ; $num2 < $dati_ca[$num1]['cp_molt']['num'] ; $num2++) {
if ((string) $catpers_molt[($num2 + 2)] == "0") {
$dati_ca[$num1]['cp_molt']['ord'][$num2] = "0";
$dati_ca[$num1]['cp_molt'][0]['esist'] = ($num2 + 1);
} # fine if ((string) $catpers_molt[($num2 + 2)] == "0")
else {
$catpers_corr = explode(">",$catpers_molt[($num2 + 2)]);
$dati_ca[$num1]['cp_molt']['ord'][$num2] = $catpers_corr[0];
$dati_ca[$num1]['cp_molt'][$num2]['lang'] = $catpers_corr[1];
$dati_ca[$num1]['cp_molt'][$num2]['n_plur'] = $catpers_corr[2];
if ($dati_cat_pers['num'] and $dati_cat_pers[$catpers_corr[0]]['langs'][$catpers_corr[1]]['n_p'] == $catpers_corr[2]) $dati_ca[$num1]['cp_molt'][$catpers_corr[0]]['esist'] = ($num2 + 1);
} # fine else if ((string) $catpers_molt[($num2 + 2)] == "0")
} # fine for $num2
} # fine if ($dati_ca[$num1]['cp_molt_int'])
...
if ($dati_ca[$num1]['letto'] == "p") {
$dati_ca[$num1]['letto'] = "s";
$dati_ca[$num1]['perc_catpers'] = "s";
} # fine ($dati_ca[$num1]['letto'] == "p")
else $dati_ca[$num1]['perc_catpers'] = "n";
...
$sett_periodipermessi = substr($dati_ca[$num1]['periodipermessi_orig'],1);
...
if ($dati_ca[$num_costo]['perc_catpers'] == "s" and strcmp($catpers_ca,"") and strcmp($dati_cat_pers[$catpers_ca]['perc'],"") and $dati_cat_pers[$catpers_ca]['perc'] != 100) {
...
if ($dati_ca[$num_costo]['cp_molt'][$cp]['esist'] and $cat_persone[$cat_persone['ord'][$cp]]['molt']) {
...
} # fine if ($dati_ca[$num_costo]['cp_molt'][$cp]['esist'] and $cat_persone[$cat_persone['ord'][$cp]]['molt'])
} # fine for $num1
...
if ($dati_ca[$num_costo]['cp_molt'][$cp]['esist'] and $num_letti_agg['catp_tot'][$num1][$cp]) {
...
} # fine if ($dati_ca[$num_costo]['cp_molt'][$cp]['esist'] and $num_letti_agg['catp_tot'][$num1][$cp])
} # fine for $num2
} # fine for $num1
} # fine if ($dati_ca[$num_costo]['modo_cp_molt'])
...
function aggiorna_letti_agg_in_periodi ($dati_ca,$num_costo,&$num_letti_agg,$idinizioperiodo,$idfineperiodo,$settimane_costo,$moltiplica,$nummoltiplica_ca,$numpersone,$dati_cat_pers=array(),$catpers_ca="") {
...
if (!$num_letti_agg['catp_tot_max'][$catpers_ca] or $num_letti_agg['catp_tot'][$num1][$catpers_ca] > $num_letti_agg['catp_tot_max'][$catpers_ca]) $num_letti_agg['catp_tot_max'][$catpers_ca] = $num_letti_agg['catp_tot'][$num1][$catpers_ca];
...
if ($dati_cat_pers['num'] and $dati_cat_pers[$catpers_corr[0]]['langs'][$catpers_corr[1]]['n_p'] == $catpers_corr[2]) $dati_cap[$numca]['cp_molt'][$catpers_corr[0]]['esist'] = ($num2 + 1);
} # fine else if ((string) $catpers_molt[($num2 + 2)] == "0")
} # fine for $num2
