...
if ($dati_ca[$dati_ca['id'][${"idcostoagg".$numca."_".$n_t}]]['mostra_numpers'] == "s" and ${"nummoltiplica_ca".$numca."_".$n_t}) ${"costoagg".$numca."_".$n_t} = "SI";
...
if ($numpersone_orig_nt[$n_t]) $num_aggiungi_letti_corr = $numpersone_orig_nt[$n_t] - ($max_capacita * $dati_r2['napp'][$tipotariffa]);
...
aggiorna_letti_agg_in_periodi($dati_ca,$num_costo,$num_letti_agg,$idinizioperiodo[$n_t],$idfineperiodo[$n_t],$settimane_costo[$numca],"",$nummoltiplica_ca_aux,$numpersone,$dati_cat_pers,${"catpers_ca".$numca."_".$n_t});
...
if ($num1 == 0 and !$numpersone_orig_nt[$n_t]) $moltiplica_costo_nr[$numca][$num1] = $moltiplica_costo[$numca];
...
} # fine if ($dati_ca[$num_costo]['cp_molt'][$cp]['esist'] and )
} # fine for $num1
...
$email2_richiedente = htmlspecialchars($email2_richiedente);
$email_cert_richiedente = htmlspecialchars($email_cert_richiedente);
...
$valuta_pagamento = str_replace(">","",$valuta_pagamento);
if (strcmp($valuta_pagamento,"")) {
...
if ($chiedi_email2 != "NO") {
if ($email2_richiedente_sb == "SI") { $apertura_tag_f = $apertura_tag_font_rosse; $chiusura_tag_f = $chiusura_tag_font_rosse; $asterisco = $fr_asterisco_rosso; }
else { $apertura_tag_f = $apertura_tag_font; $chiusura_tag_f = $chiusura_tag_font; $asterisco = $fr_asterisco_nero; }
...
if ($email_cert_richiedente_sb == "SI") { $apertura_tag_f = $apertura_tag_font_rosse; $chiusura_tag_f = $chiusura_tag_font_rosse; $asterisco = $fr_asterisco_rosso; }
else { $apertura_tag_f = $apertura_tag_font; $chiusura_tag_f = $chiusura_tag_font; $asterisco = $fr_asterisco_nero; }
...
$altre_valute = altre_valute($utente_liste);
if ($altre_valute['num']) {
if (!strcmp($valuta_pagamento,"")) $sel = " selected";
else $sel = "";
...
if ($altre_valute[$num1]['nome'] == $valuta_pagamento) $sel = " selected";
else $sel = "";
...
for ($num2 = 0 ; $num2 < $dati_cat_pers['num'] ; $num2++) {
if (strcmp($cat_persone_nr[$n_t][($num1 - 1)][$cat_persone_nr[$n_t][($num1 - 1)]['ord'][$num2]]['molt'],"")) $cat_pers_corr = $cat_persone_nr[$n_t][($num1 - 1)][$cat_persone_nr[$n_t][($num1 - 1)]['ord'][$num2]]['molt'];
...
if (($mostra_quadro_disp == "SI" or ($mostra_quadro_disp == "SI_NO" and $mostra_c_e_disp != "SI")) and $manda_mail != "SI") {
...
if (!$framed) $testo_costi_agg .= "<table><tr><td style=\"height: 10px;\"></td></tr></table>";
...
if ($mostra_numpers) $$testo_costo .= ";</td><td style=\"width: 20px;\"></td><td><input type=\"hidden\" name=\"idcostoagg$numcostiagg"."_$n_t\" value=\"$idcostoagg\">$nomecosto_imposto:";
...
if ($mostra_numpers) $$testo_categ .= ";</td><td style=\"width: 20px;\"></td><td>$categoria_vedi:";
...
} # fine else if (${$nummoltiplica_ca."_".$n_t})
...
if ($dati_cat_pers['num'] or $testo_catpers_ca) echo "$testo_catpers_ca.</td></tr></table>";
echo $chiusura_tag_font;
